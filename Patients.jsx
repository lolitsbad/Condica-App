...
export default function Patients() {
  ...
  async function save(form, editId) {
    ...
    else await supabase.from('patients').insert({ ...form, org_id: org.id });
    setModal(null);
    load();
  }
  ...
  async function eraseCompletely(p) {
    ...
    await supabase.from('audit_log').insert({ org_id: org.id, action: 'gdpr', details: `Date șterse definitiv pentru ${p.name} (drept la ștergere GDPR)` });
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-semibold" style={{ color: C.text, fontFamily: "'Fraunces', serif" }}>Pacienți</h1>
      ...
      {patients.length === 0 ? (
        ...
        <div className="grid grid-cols-2 gap-4">
          {patients.map((p) => (
            <div key={p.id} className="rounded-2xl p-4 shadow-sm condica-card" style={{ backgroundColor: C.surface, border: `1px solid ${C.line}` }}>
