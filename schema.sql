...
begin
  ...
  insert into organizations (name, slug, plan) values (v_org_name, v_slug, coalesce(new.raw_user_meta_data->>'plan', 'solo')) returning id into v_org_id;
...
-- Creează pacientul din pagina publică (dacă nu există deja), fără să expună
-- date despre alți pacienți către vizitatori neautentificați.
create or replace function upsert_patient_from_booking(p_org_id uuid, p_name text, p_phone text)
returns void
language plpgsql
security definer
set search_path = public
as $$
begin
  if not exists (select 1 from patients where org_id = p_org_id and lower(name) = lower(p_name)) then
    insert into patients (org_id, name, phone) values (p_org_id, p_name, p_phone);
  end if;
end;
$$;
