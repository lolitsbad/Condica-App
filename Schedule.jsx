...
export default function Schedule() {
  ...
  return (
    <div className="p-8">
      <h1 className="text-2xl font-semibold mb-1" style={{ color: C.text, fontFamily: "'Fraunces', serif" }}>Program de lucru</h1>
      <p className="text-sm mb-6" style={{ color: C.inkSoft }}>Pacienții văd și se pot programa doar în intervalele libere de mai jos.</p>

      <div className="rounded-2xl p-4 mb-6" style={{ backgroundColor: C.surface, border: `1px solid ${C.line}` }}>
        <label className="block text-sm font-medium mb-2" style={{ color: C.text }}>Email pentru notificări de programări noi</label>
      ...
      <div className="rounded-2xl p-4 mb-6" style={{ backgroundColor: C.surface, border: `1px solid ${C.line}` }}>
        <div className="flex items-center justify-between flex-wrap gap-3">
          <label className="flex items-center gap-2 text-sm font-medium" style={{ color: C.text }}>
      ...
      <div className="rounded-2xl overflow-hidden mb-8" style={{ border: `1px solid ${C.line}` }}>
        {DAY_ORDER.map((dow, i) => {
          ...
          return (
            <div key={dow} className="flex items-center gap-4 px-4 py-3 flex-wrap" style={{ backgroundColor: C.surface, borderTop: i ? `1px solid ${C.line}` : 'none' }}>
              <label className="flex items-center gap-2 w-32 shrink-0">
                ...
                <span className="text-sm font-medium" style={{ color: C.text }}>{DAY_LABEL[dow]}</span>
              </label>
              {cfg.enabled ? (
                <>
                  <div className="flex items-center gap-2 text-sm" style={{ color: C.inkSoft }}>
      ...
      {blockedDates.length === 0 ? (
        ...
        <div className="rounded-2xl overflow-hidden" style={{ border: `1px solid ${C.line}` }}>
          {blockedDates.map((b, i) => (
            <div key={b.id} className="flex items-center gap-3 px-4 py-3" style={{ backgroundColor: C.surface, borderTop: i ? `1px solid ${C.line}` : 'none' }}>
              <Ban size={14} style={{ color: C.coral }} />
              <span className="text-sm font-medium" style={{ color: C.text }}>{formatDateShort(b.date)}</span>
