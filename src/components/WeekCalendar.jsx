...
export default function WeekCalendar({ appointments, workingHours, weekStart, onSelectAppointment }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden bg-white" style={{ border: `1px solid ${C.line}` }}>
                ...
                {dayAppts.map((a) => {
                  ...
                  return (
                    <button key={a.id} onClick={() => onSelectAppointment(a)}
                      ...
                      <div className="text-[10px] font-semibold truncate flex items-center gap-1" style={{ color: C.ink }}><SourceIcon source={a.source} />{a.time} · {a.patient_name}</div>
