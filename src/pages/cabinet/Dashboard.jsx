...
export default function Dashboard() {
  ...
  return (
    <div className="p-8">
      <h1 className="text-2xl font-semibold mb-1" style={{ color: C.ink, fontFamily: "'Fraunces', serif" }}>Bună ziua, {activeDoctor?.name || 'Doctore'}</h1>
      ...
      {todays.length === 0 ? (
        <div className="rounded-2xl p-6 text-sm" style={{ backgroundColor: '#fff', border: `1px solid ${C.line}`, color: C.inkSoft }}>Nicio programare azi.</div>
      ) : (
        <div className="rounded-2xl overflow-hidden" style={{ border: `1px solid ${C.line}` }}>
          {todays.map((a, i) => (
            <div key={a.id} className="flex items-center gap-4 px-4 py-3 bg-white" style={{ borderTop: i ? `1px solid ${C.line}` : 'none' }}>
              <div className="w-14 text-sm font-semibold" style={{ color: C.ink }}>{a.time}</div>
