...
import { LayoutDashboard, CalendarDays, Users, Wallet, Settings, Sparkles, ShieldCheck, Plus, LogOut, Activity } from 'lucide-react';
...
export default function CabinetLayout() {
  ...
  async function loadDoctors() {
    ...
    if (!error && data) {
      setDoctors(data);
      setActiveDoctorId((prev) => prev && data.some((d) => d.id === prev) ? prev : data[0]?.id);
    }
    setReady(true);
  }

  useEffect(() => { if (session) loadDoctors(); }, [session]); // eslint-disable-line

  async function addDoctor() {
    if (!doctorForm.name.trim()) return;
    const color = DOCTOR_COLORS[doctors.length % DOCTOR_COLORS.length];
    const { data, error } = await supabase.from('doctors').insert({ name: doctorForm.name, specialty: doctorForm.specialty, color }).select().single();
    if (!error && data) {
      await supabase.from('working_hours').insert(
        [1, 2, 3, 4, 5, 6, 0].map((d) => ({ doctor_id: data.id, day_of_week: d, enabled: d >= 1 && d <= 5, start_time: '09:00', end_time: d === 5 ? '15:00' : '17:00', slot_minutes: 20 }))
      );
      await loadDoctors();
  ...
  return (
    <div className="min-h-screen flex" style={{ background: `radial-gradient(1200px 500px at 50% -10%, #ffffff 0%, ${C.paper} 65%)`, fontFamily: "'IBM Plex Sans', sans-serif" }}>
      <div className="w-56 shrink-0 flex flex-col py-6 px-4 overflow-y-auto" style={{ backgroundColor: C.ink }}>
        ...
        <nav className="flex flex-col gap-1">
          {navItems.map(({ to, label, Icon, end }) => (
            <NavLink key={to} to={to} end={end} className={({ isActive }) => 'flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium'}
        ...
        <div className="mt-auto px-2 pt-4">
          <div className="flex items-center gap-1.5 text-xs mb-3" style={{ color: 'rgba(255,255,255,0.4)' }}>
            <ShieldCheck size={13} /> Date criptate · GDPR
          </div>
      ...
      <div className="flex-1 min-w-0 overflow-y-auto">
        <Outlet context={{ doctors, activeDoctorId, refreshDoctors: loadDoctors }} />
      </div>
