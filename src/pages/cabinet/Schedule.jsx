...
export default function Schedule() {
  const { activeDoctorId } = useOutletContext();
  ...
  async function load() {
    ...
    setBlockedDates(bd || []);
    const { data: rs } = await supabase.from('reminder_settings').select('*').eq('id', 1).maybeSingle();
  ...
  async function updateDay(dow, patch) {
    ...
    await supabase.from('working_hours').upsert({
      doctor_id: activeDoctorId, day_of_week: dow, enabled: cfg.enabled, start_time: cfg.start,
  ...
  async function addBlocked() {
    ...
    await supabase.from('blocked_dates').insert({ ...blockForm, doctor_id: activeDoctorId });
  ...
  async function updateReminderSettings(patch) {
    ...
    await supabase.from('reminder_settings').update(next).eq('id', 1);
  }
  ...
  return (
    <div className="p-8">
      ...
      <div className="rounded-2xl p-4 mb-6" style={{ backgroundColor: '#fff', border: `1px solid ${C.line}` }}>
        ...
        <p className="text-[11px] mt-2" style={{ color: C.inkSoft }}>Pentru trimitere reală în fundal (nu doar cât timp e cineva conectat), programează funcția din <code>/api/assistant</code> ca job periodic (Vercel Cron) care verifică programările apropiate.</p>
      </div>
