  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-semibold" style={{ color: C.text, fontFamily: "'Fraunces', serif" }}>Facturare</h1>
      ...
      {invoices.length === 0 ? (
        <div className="rounded-2xl p-8 text-center" style={{ backgroundColor: C.surface, border: `1.5px dashed ${C.line}` }}>
          <p className="text-sm" style={{ color: C.inkSoft }}>Nicio factură emisă încă.</p>
        </div>
      ) : (
        <div className="rounded-2xl overflow-hidden" style={{ border: `1px solid ${C.line}` }}>
          {invoices.map((inv, i) => (
            <div key={inv.id} className="flex items-center gap-4 px-4 py-3 flex-wrap" style={{ backgroundColor: C.surface, borderTop: i ? `1px solid ${C.line}` : 'none' }}>
              <div className="w-20 text-xs font-semibold" style={{ color: C.inkSoft }}>{inv.number}</div>
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium truncate" style={{ color: C.text }}>{inv.patient_name}</div>
                <div className="text-xs truncate" style={{ color: C.inkSoft }}>{inv.description} · {formatDateShort(inv.date)}</div>
              </div>
              <div className="text-sm font-semibold" style={{ color: C.text }}>{Number(inv.price || 0).toLocaleString('ro-RO')} RON</div>
