...
export default function Billing() {
  ...
  async function save(form, editId) {
    if (editId) {
      ...
      await supabase.from('invoices').insert({ ...form, number, doctor_id: activeDoctorId });
      await supabase.from('audit_log').insert({ action: 'factura', details: `Factură ${number} emisă pentru ${form.patient_name} · ${form.price} RON` });
    }
    setModal(null);
    load();
  }
