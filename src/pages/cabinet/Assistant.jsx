...
Reguli: dacă cererea nu conține suficiente detalii, folosește action "none" și cere lămuriri. Interpretează expresii relative ("mâine", "joi viitor") față de data de azi. Dacă acțiunea este create_appointment, reply trebuie să confirme programarea.`;
    ...
    try {
      ...
      if (parsed.action === 'create_appointment' && parsed.appointment?.patientName && parsed.appointment?.date && parsed.appointment?.time) {
        ...
        await supabase.from('appointments').insert({
          doctor_id: activeDoctorId, patient_name: a.patientName, phone: a.phone || '', date: a.date, time: a.time,
  ...
  return (
    <div className="p-8 flex flex-col h-screen max-w-2xl">
      <h1 className="text-2xl font-semibold mb-1" style={{ color: C.ink, fontFamily: "'Fraunces', serif" }}>Asistent AI</h1>
      ...
      <div className="flex-1 rounded-2xl bg-white p-4 overflow-y-auto mb-3" style={{ border: `1px solid ${C.line}` }}>
        {messages.map((m, i) => (
      ...
      <div className="flex items-end gap-2">
        <textarea value={input} onChange={(e) => setInput(e.target.value)}
          ...
          placeholder="Scrie un mesaj..." rows={1} className="flex-1 resize-none rounded-xl px-4 py-2.5 text-sm outline-none" style={{ border: `1px solid ${C.line}`, backgroundColor: '#fff', color: C.ink }} />
