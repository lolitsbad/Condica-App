  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
        <h1 className="text-2xl font-semibold" style={{ color: C.text, fontFamily: "'Fraunces', serif" }}>Programări</h1>
      ...
      {apptView === 'week' && (
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium" style={{ color: C.text }}>{formatDateShort(toDateStr(weekStart))} – {formatDateShort(toDateStr(new Date(weekStart.getTime() + 6 * 86400000)))}</span>
            ...
            <div className="rounded-2xl overflow-hidden" style={{ border: `1px solid ${C.line}` }}>
              {grouped[date].map((a, i) => (
                <div key={a.id} className="flex items-center gap-4 px-4 py-3" style={{ backgroundColor: C.surface, borderTop: i ? `1px solid ${C.line}` : 'none' }}>
                  <div className="w-14 text-sm font-semibold flex items-center gap-1" style={{ color: C.text }}><Clock size={13} style={{ color: C.inkSoft }} /> {a.time}</div>
