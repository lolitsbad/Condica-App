    ...
    <div className="min-h-screen p-6 md:p-10" style={{ background: `radial-gradient(1200px 500px at 50% -10%, #ffffff 0%, ${C.paper} 65%)`, fontFamily: "'IBM Plex Sans', sans-serif" }}>
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center gap-2.5 justify-center mb-8">
          ...
          <span className="text-xl font-semibold" style={{ color: C.text, fontFamily: "'Fraunces', serif" }}>Condicamed</span>
        </div>
        ...
        <p className="text-sm text-center mb-8" style={{ color: C.textSoft }}>Abonament lunar, fără costuri ascunse. Anulezi oricând.</p>
          ...
          {PLANS.map((p) => (
            <div key={p.id} className="rounded-3xl p-6 condica-card" style={{ backgroundColor: C.surface, border: p.highlight ? `2px solid ${C.violet}` : `1px solid ${C.line}` }}>
              ...
              <div className="mb-5">
                ...
                <span className="text-sm" style={{ color: C.textSoft }}>{p.period}</span>
              </div>
              ...
              <button onClick={() => proceedToCheckout(p.id)} disabled={loadingPlan === p.id}
        ...
        <div className="rounded-3xl p-6 text-center" style={{ backgroundColor: C.paperDeep }}>
          ...
          <a href="mailto:contact@condicamed.com" className="inline-block px-5 py-2.5 rounded-lg text-sm font-medium" style={{ backgroundColor: C.surface, color: C.text, border: `1px solid ${C.line}` }}>Contactează-ne</a>
        </div>
      </div>
    </div>
  );
