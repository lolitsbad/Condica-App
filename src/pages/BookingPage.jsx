...
import { PulseDivider, StatusBadge, Field, inputStyle } from '../components/ui';
...
export default function BookingPage() {
  ...
  useEffect(() => {
    (async () => {
      const { data: docs } = await supabase.from('doctors').select('*').order('created_at');
      setDoctors(docs || []);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setBlockedDates(bd || []);
      const { data: appts } = await supabase.from('appointments').select('id,date,time,duration,status').eq('doctor_id', selectedDoctorId);
      setAppointments(appts || []);
    })();
  }, [selectedDoctorId]);

  async function submitBooking() {
    if (!form.name.trim() || !form.phone.trim() || !selectedSlot || !consent) return;
    const dow = new Date(selectedDate + 'T00:00:00').getDay();
    const duration = (workingHours[dow] && workingHours[dow].slot) || 30;
    await supabase.from('appointments').insert({
      doctor_id: selectedDoctorId, patient_name: form.name.trim(), phone: form.phone.trim(),
      date: selectedDate, time: selectedSlot, duration, reason: form.reason.trim(), status: 'confirmed', source: 'online',
    });
    // fire-and-forget confirmation via serverless assistant (best-effort, ignored if it fails)
    fetch('/api/assistant', {
      ...
      supabase.from('notifications').insert({ type: 'created', patient_name: form.name.trim(), date: selectedDate, time: selectedSlot, message: d.text || '' });
    }).catch(() => {});
  ...
  async function search() {
    setSearched(true);
    const { data } = await supabase.rpc('get_my_appointments', { p_phone: lookupPhone.trim() });
    setMyAppts(data || []);
  }
  async function cancelMine(id) {
    await supabase.rpc('cancel_my_appointment', { p_id: id, p_phone: lookupPhone.trim() });
    setConfirmCancelId(null);
    search();
  }
  async function rescheduleMine(id) {
    await supabase.rpc('reschedule_my_appointment', { p_id: id, p_phone: lookupPhone.trim(), p_date: rescheduleSel.date, p_time: rescheduleSel.time });
  ...
  if (!loaded) return <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: C.paper }} />;
  ...
  if (confirmed) {
    return (
      <div className="min-h-screen flex items-center justify-center p-6" style={{ background: `radial-gradient(1200px 500px at 50% -10%, #ffffff 0%, ${C.paper} 65%)` }}>
        <div className="max-w-md w-full text-center rounded-3xl p-8 shadow-xl condica-fade" style={{ backgroundColor: '#fff', border: `1px solid ${C.line}` }}>
          ...
          <h2 className="text-xl font-semibold mb-2" style={{ color: C.ink, fontFamily: "'Fraunces', serif" }}>Programare confirmată</h2>
          <p className="text-sm mb-1" style={{ color: C.inkSoft }}>{confirmed.name}, te așteptăm</p>
          <p className="text-sm mb-6 font-medium" style={{ color: C.ink }}>{formatDateLong(confirmed.date)} · ora {confirmed.time}</p>
    ...
    <div className="min-h-screen" style={{ background: `radial-gradient(1200px 500px at 50% -10%, #ffffff 0%, ${C.paper} 65%)`, fontFamily: "'IBM Plex Sans', sans-serif" }}>
      <div className="p-6 md:p-10 max-w-2xl mx-auto w-full">
        <div className="text-center mb-6 condica-fade">
          <div className="text-[11px] font-semibold uppercase tracking-widest mb-2" style={{ color: C.violet }}>Programare online</div>
          <h1 className="text-3xl font-semibold mb-2" style={{ color: C.ink, fontFamily: "'Fraunces', serif", letterSpacing: '-0.01em' }}>Alege-ți ora, în câteva secunde</h1>
        ...
        {tab === 'book' ? (
          <>
            ...
            {selectedSlot && (
              <div className="rounded-2xl p-5 mt-6" style={{ backgroundColor: '#fff', border: `1px solid ${C.line}` }}>
                <h3 className="text-sm font-semibold mb-3" style={{ color: C.ink }}>Datele tale</h3>
          ...
          <div>
            ...
            {searched && myAppts.length === 0 && (
              <div className="rounded-2xl p-6 text-center text-sm" style={{ backgroundColor: '#fff', border: `1px solid ${C.line}`, color: C.inkSoft }}>Nu am găsit nicio programare activă pentru acest număr.</div>
            )}
            {myAppts.map((a) => (
              <div key={a.id} className="rounded-2xl p-4 mb-3 bg-white" style={{ border: `1px solid ${C.line}` }}>
                <div className="flex items-center justify-between mb-1">
                  <div className="text-sm font-semibold" style={{ color: C.ink }}>{formatDateLong(a.date)} · ora {a.time}</div>
